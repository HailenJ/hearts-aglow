...
> **OWN-WORLD:** Void #08070b under three gaussian light blooms cycling violet #c48aff to amber #ffa878 over ~12 minutes, dithered so no edge ever forms. Signal cyan #dff4ff on focus, active state, live data, and primary interactive affordance (.hero__cta-go, .game__submit, .game__store) — nowhere else. Text: --text rgba(255,255,255,0.95), --text-dim rgba(255,255,255,0.80), --text-faint rgba(255,255,255,0.66), raised during the build so all three clear WCAG AA 4.5:1 against the light field's brightest phase, measured at rgb(90,64,103). Anybody (display), Archivo (body), Martian Mono (data). Hairline glass panes, 3px radii, film grain over everything.
...
1. **`--bloom-warm-2` and `--bloom-deep` are not real CSS tokens.** The task brief and the `index.html` thesis describe a bloom cycling "violet #c48aff to amber #ffa878," and the shader (`LightField.jsx:23-26`) does define all three colors as GLSL constants (`VIOLET = #c48aff`, `AMBER = #ffa878`, `DEEP = #483478`). But `globals.css` only mirrors one of them into a custom property — `--bloom-warm: #c48aff` (`globals.css:10`) — despite its own comment claiming the bloom tokens are "mirrored from the shader constants" (plural, `globals.css:9`). Amber and deep exist only inside the shader; there is no `--bloom-warm-2` or `--bloom-deep` CSS variable anywhere in the codebase (confirmed by grep). Any CSS that wants the amber or deep bloom color today has no token to reach for.
2. **A fourth, untokenized warm accent color is used throughout Works and Contact.** `rgba(232, 149, 120, …)` (`#e89578`) appears 15 times across `.about__text a`, `.works__item--featured::after`, `.works__item:hover`, `.works__detail-link`, `.contact__primary`, and others — but this hex matches neither `--bloom-warm` (#c48aff, violet) nor the shader's `AMBER` (#ffa878) constant. It reads as a third, hand-picked warm color that was never promoted to a token and doesn't correspond to anything the shader actually renders.
3. **Two hover states use `--signal` outside the four sanctioned Signal Discipline buckets.** `.window__btn:hover` (`globals.css:354`) and `.player__close:hover` (`globals.css:1111`) turn plain chrome icon buttons (minimize/maximize/close, player-close) signal-cyan on hover. These are not focus, not a persisted active state, not live data, and not one of the three named primary affordances (`.hero__cta-go`, `.game__submit`, `.game__store`). Similarly, `.works__play` (`globals.css:1053,1064`) shares its entire rule with `.game__submit` — including resting `--signal` text — but is not named in the `index.html` thesis's list of three. Whether these are meant to fall under "primary interactive affordance" by extension, or are drift from the stated rule, is unresolved in the code as written.
